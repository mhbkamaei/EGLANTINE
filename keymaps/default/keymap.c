#include "kb.h"

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

	[0] = LAYOUT(
		MO(1), KC_Q, KC_W, KC_E, KC_R, KC_T,
		MO(2), KC_A, KC_S, KC_D, KC_F, KC_G,
		KC_PIPE, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_SLSH,
		KC_BSPC, KC_SPC, KC_ENT),

	[1] = LAYOUT(
		KC_TRNS, KC_TRNS, KC_7, KC_8, KC_9, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_4, KC_5, KC_6, KC_TRNS,
		KC_TRNS, KC_0, KC_1, KC_2, KC_3, KC_TRNS, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_TRNS),

	[2] = LAYOUT(
		KC_TRNS, KC_TRNS, KC_GRV, KC_GRV, KC_GRV, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_COMM, KC_QUOT, KC_DOT, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_RBRC, KC_DOT, KC_COMM, KC_TRNS, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_TRNS),

	[3] = LAYOUT(
		KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
		KC_TRNS, KC_TRNS, KC_TRNS),

};
